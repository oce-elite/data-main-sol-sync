...
void Player::addInFightTicks(bool pzlock /*= false*/) {
	...
	// VIP Check to Reduce Aggression Time
	int inFightTime = g_configManager().getNumber(PZ_LOCKED);
	if (isVip()) {
		inFightTime /= 2; // Reduce to 30 seconds (Half of normal PZ_LOCKED)
	}

	safeCall([this, inFightTime] {
		addCondition(Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_INFIGHT, inFightTime));
	});
}

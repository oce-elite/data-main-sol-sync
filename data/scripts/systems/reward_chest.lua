dofile("data/libs/fiendish_config.lua")
	...
	if monsterType and monsterType:isRewardBoss() then
		if not corpse or not corpse.isContainer or not corpse:isContainer() then
			if corpse.getId then
				logger.debug("[bossDeath.onDeath] Boss {} has a corpse (id: {}, name: {}), but it is not a container.",
					creature:getName(), corpse:getId(), corpse:getName())
			else
				logger.debug("[bossDeath.onDeath] Boss {} does not have a corpse or corpse not found at position {}",
		...
		for guid, stats in pairs(info) do
			...
			local damageOut, damageIn, healing = (stats.damageOut or 0) + (part and part.total or 0), stats.damageIn or 0,
				stats.healing or 0
		...
		for _, con in ipairs(scores) do
			local score = (con.damageOut / totalDamageOut) + (con.damageIn / totalDamageIn) +
				(con.healing / totalHealing)
			-- Normalize to 0-1
			con.score = score / 3
			if score ~= 0 then
		...
		for _, con in ipairs(scores) do
			...
			if con.score ~= 0 then
				...
				if con.player then
					...
					local lootMessage = ("The following items dropped by %s are available in your reward chest: %s")
						:format(creature:getName(), reward:getContentDescription(collorMessage))
					if rolls > 1 then
...
function bossParticipation.onHealthChange(creature, attacker, primaryDamage, primaryType, secondaryDamage, secondaryType,
										  origin)

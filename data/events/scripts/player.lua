...
local taints = {
	...
	if taints.monsters[monsterName] and taintLevel and taintLevel > 0 then
    	local count = math.min(taintLevel, 5) -- Corrected max limit
    	taints_xpboost = taints.xpboost[count] or 0 -- Ensure a valid lookup
	end
	...
	-- Return final experience value
		return (exp * (1 + xpBoostPercent / 100 + lowLevelBonusExp / 100 + taints_xpboost / 100)) * staminaBonusXp * baseRateExp
	end

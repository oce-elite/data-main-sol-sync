...
local removeTaint = TalkAction("/removetaint")

function removeTaint.onSay(player, words, param)
	local split = param:split(",")
	local target = Player(split[1])

	if not target then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Player is offline or does not exist.")
		return false
	end

	local taintLevel = tonumber(split[2])
	if not taintLevel then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Invalid taint level.")
		return false
	end

	local taintName = player:getTaintNameByNumber(taintLevel)
	if not taintName then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Taint level does not exist.")
		return false
	end

	local soulWarQuest = target:soulWarQuestKV()
	if not soulWarQuest then
		player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Error: Cannot access target's Soul War quest data.")
		return false
	end

	soulWarQuest:remove(taintName)
	target:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Your taint level: " .. taintLevel .. " (" .. taintName .. ") has been removed.")
	player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "Removed taint level: " .. taintLevel .. " (" .. taintName .. ") from player: " .. target:getName())

	return true
end

removeTaint:separator(" ")
removeTaint:groupType("god")
removeTaint:register()
